...
期望键 MUST 通过静态扫描收集，不得依赖运行时数据库：宿主来自`manifest/sql`中`sys_config` seed 与宿主声明的`sys.*`/`demo.*`（及`SysConfigKey`）常量；插件仅当`plugin.yaml`中`i18n.enabled: true`时收集其`SysConfigKey`常量。宿主键 MUST 出现在宿主`manifest/i18n/<locale>/`；插件键 MUST 出现在该插件`manifest/i18n/<locale>/`。
