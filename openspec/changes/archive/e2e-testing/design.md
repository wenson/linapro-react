...
- `ci-shard host|plugin|plugin-full-extra <name>`：CI 分片入口，解析 manifest `ciShards` 后复用同一套 parallel/serial 拆分与 worker 环境变量。

execution manifest 是 smoke 文件、module scope、串行边界、隔离类别、allowlist 以及 **CI 分片治理（ciShards）** 的唯一事实来源。full、module 与 ci-shard 模式都采用同一套“并行池 + 串行池”拆分逻辑，module/分片运行不会绕过隔离规则。host-only module 在解析到 `plugins`、`plugin:<plugin-id>` 或任何需要 `apps/lina-plugins` 的 scope 时必须直接失败，避免在错误环境中产生伪失败。
...
墙钟优化的主杠杆不是把 serial 用例强行改 parallel（高 flaky 风险），而是让「模块内 serial」在「模块间 job 并行」下仍安全。为此在 `config/execution-manifest.json` 增加 `ciShards`：
...
- **host**：显式 `entries`，与能力边界对齐；验证器要求全集恰好等于 host-only 可发现宿主 TC，禁止漏测与重复覆盖。
- **plugin**：`targetShardCount` + 按源码插件 TC 文件数（或 `weightOverrides[pluginId]`）的 bin-pack；装箱单元是「每个源码插件的 TC 集合」，不在根目录硬编码官方插件业务别名。
- **pluginFullExtra**：plugin-full 环境特有的宿主框架分片（如 `extension:plugin`），与 host-only 语义区分保留。

规划与发射工具（`emit-ci-shards`）在本地与 CI 中输出稳定的分片 `name` 与可直接执行的 `command`。verification suite 增加轻量 `plan-e2e-shards` job：checkout（plugin 侧需要 recursive submodule）后生成 host/plugin matrix JSON，host-only / plugin-full E2E 使用 `strategy.matrix.shard: fromJson(...)`，避免 YAML 与插件清单双源漂移。

各分片使用独立 runner（独立 PostgreSQL 与服务进程）、独立 artifact 名称；任一分片失败阻断完整 verification suite 下游（含镜像发布）。runner 在 full、module 和 shard 模式下都输出并行文件数、串行文件数、worker 数量和串行隔离类别摘要。PostgreSQL service health check 使用显式 `pg_isready -U postgres -d linapro`，避免依赖 runner OS 用户产生无效健康检查噪声。

## 并行 worker 与安装缓存

- Nightly 等完整 E2E 调用方默认 `e2e-parallel-workers ≥ 2`，与 manifest `ciShards.defaults.parallelWorkers` 对齐；仅加速 parallel 池，serial 池保持 workers=1。
- 分片后的墙钟收益主要来自 job 并行，workers 为叠加收益；不在本设计中强制改写 serial 声明。
- E2E job 启用 Go module cache，并缓存 Playwright 浏览器目录（如 `~/.cache/ms-playwright`），降低多 job 重复 setup 成本。
- Main CI / Release 仍默认关闭完整 E2E；后续若需要自适应耗时分片，仅扩展 packing 策略与 weight 来源，不改 CI 契约形状。
...
| 风险 | 缓解 |
| --- | --- |
| 分片增多导致 runner 分钟数上升 | 墙钟下降优先；`targetShardCount` 可调；Main CI 仍关完整 E2E |
| bin-pack 仍可能偶发不均 | `weightOverrides`；后续可接入历史耗时 |
| 动态 matrix 在插件 submodule 失败时无分片 | require-plugin-e2e 与校验失败阻断 |
| 独立 job 重复 setup | Go/Playwright 缓存降低安装成本；不与单 job 串行 40m 同量级 |
| 并行提速暴露更多真实 flaky | 隔离类别、baseline 与最小生产修正闭环 |

## Cross-Domain Impacts

- E2E 治理迭代调整过 `project-setup` 中的 `make env.setup` Playwright headless shell 安装语义，以及测试入口对管理工作台路径和源码插件公开路径的区分；当前项目初始化契约由 `openspec/specs/project-setup/spec.md` 承载，历史 owner 为 `archive/foundation`。
