...
import { MobileRecordActions, MobileRecordCard, MobileRecordField, MobileRecordFields, MobileRecordList, MobileRecordTitle, requestTenantImpersonation, useLinaPluginHost } from "@linapro/plugin-ui";
...
export default function TenantManagement() {
  ...
  const columns: ColumnProps<PlatformTenant>[] = [
    ...
    { dataIndex: "status", render: (_, row) => renderStatus(row), title: host.t("pages.common.status"), width: 190 },
    ...
    { fixed: "right", render: (_, row) => renderActions(row), title: host.t("pages.common.actions"), width: 300 },
  ];
  ...
  return <section className="tenant-core-page" data-testid="platform-tenants-page">
    ...
    <Card><Form<PlatformTenantListParams> className="tenant-core-search" id="tenant-filter-form" layout="horizontal" onReset={reset} onSubmit={(values) => setParams((current) => ({ ...current, ...values, pageNum: 1 }))}><Form.Input data-testid="tenant-search-code" field="code" id="tenant-filter-code" label={host.t("plugin.linapro-tenant-core.fields.code")} /><Form.Input data-testid="tenant-search-name" field="name" id="tenant-filter-name" label={host.t("plugin.linapro-tenant-core.fields.name")} /><Form.Select data-testid="tenant-search-status" field="status" id="tenant-filter-status" label={host.t("pages.common.status")} optionList={["active", "suspended"].map((value) => ({ label: host.t(`plugin.linapro-tenant-core.status.${value}`), value }))} /><Space><Button htmlType="reset">{host.t("pages.common.reset")}</Button><Button htmlType="submit" theme="solid" type="primary">{host.t("pages.common.search")}</Button></Space></Form></Card>
    <Card><div className="tenant-core-toolbar"><Space>{allowed(host.permissions, "system:tenant:remove") ? <Button data-testid="tenant-batch-delete" disabled={!selected.length} loading={pending === "delete"} onClick={batchDelete} type="danger">{host.t("pages.common.delete")}</Button> : null}{allowed(host.permissions, "system:tenant:add") ? <Button data-testid="tenant-create" onClick={() => setEditor("new")} theme="solid" type="primary">{host.t("pages.common.add")}</Button> : null}</Space></div><div className="responsive-desktop-table" data-testid="platform-tenants-table"><Table<PlatformTenant> columns={columns} dataSource={rows} loading={loading} pagination={{ currentPage: params.pageNum, onChange: (pageNum) => setParams((current) => ({ ...current, pageNum })), pageSize: params.pageSize, showTotal: true, total }} rowKey="id" rowSelection={{ onChange: (keys) => setSelected((keys ?? []).map(Number)), selectedRowKeys: selected }} scroll={{ x: 1100 }} /></div><MobileRecordList testId="platform-tenants-mobile-list">{rows.map((row) => <MobileRecordCard key={row.id} testId={`platform-tenant-mobile-card-${row.id}`}><MobileRecordTitle>{row.name}</MobileRecordTitle><MobileRecordFields><MobileRecordField label={host.t("plugin.linapro-tenant-core.fields.code")} value={row.code} /><MobileRecordField label={host.t("pages.common.status")} value={renderStatus(row)} /><MobileRecordField label={host.t("pages.common.createdAt")} value={timestamp(row.createdAt, host.locale)} /></MobileRecordFields><MobileRecordActions>{renderActions(row)}</MobileRecordActions></MobileRecordCard>)}</MobileRecordList></Card>
