...
test.describe("TC002 运行时国际化切换", () => {
  ...
  test("TC-2b: 已登录会话切换语言后菜单、版本信息与字典动态元数据会同步刷新", async ({
    ...
    const systemInfoContent = adminPage.getByTestId("system-info-page");
    await expect(
      systemInfoContent.getByText("About the project", { exact: true }),
    ).toBeVisible();
    await expect(
      systemInfoContent.getByText("Project name", { exact: true }),
    ).toBeVisible();
    await expect(
