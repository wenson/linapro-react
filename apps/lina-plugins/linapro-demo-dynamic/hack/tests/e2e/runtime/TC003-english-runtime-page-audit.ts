...
test.describe("TC003 英文运行时页面巡检", () => {
  ...
  test("TC-3a: 英文环境下动态插件管理列表元数据保持英文", async ({
    ...
    await expect(pluginPage.pluginDescriptionCell(pluginID)).toContainText(
      "Dynamic wasm sample that demonstrates an isolated hosted page, governed bridge access, plugin-owned SQL CRUD, and attachments.",
    );
  ...
  test("TC-3c: 英文环境下动态插件页面与独立页种子内容保持英文", async ({
    ...
    await expect(
      pluginPage.pluginHostedFrame().getByText(
    ...
    await expect(
      standalonePage.getByText("Dynamic Plugin Demo Is Live", {
        exact: true,
      }),
    ).toBeVisible();
    await expect(
      standalonePage.getByText(
        "This page runs in an isolated iframe",
        { exact: false },
      ),
    ).toBeVisible();
