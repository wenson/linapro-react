...
function cleanupRuntimePluginRows() {
  ...
  execPgSQLStatements([
    `DELETE FROM sys_role_menu WHERE menu_id IN (SELECT id FROM sys_menu WHERE menu_key IN ('${pgEscapeLiteral(iframeMenuKey)}', '${pgEscapeLiteral(isolatedMenuKey)}', '${pgEscapeLiteral(newWindowMenuKey)}'));`,
    `DELETE FROM sys_menu WHERE menu_key IN ('${pgEscapeLiteral(iframeMenuKey)}', '${pgEscapeLiteral(isolatedMenuKey)}', '${pgEscapeLiteral(newWindowMenuKey)}');`,
  ...
  return [
    {
      key: iframeMenuKey,
      name: iframeMenuName,
      path: "/extension/runtime-e2e-iframe",
      ...
      query: {
        ...
        pluginAssetUrl: hostedAssetPath,
      },
      remark: "Runtime-hosted iframe entry used by Playwright verification.",
    },
    {
      key: isolatedMenuKey,
      name: isolatedMenuName,
      path: "/extension/runtime-e2e-isolated",
      component: "system/plugin/dynamic-page",
      perms: "plugin-dev-dynamic-e2e:isolated:view",
      icon: "ant-design:deployment-unit-outlined",
      type: "M",
      sort: -2,
      query: {
        pluginAccessMode: "iframe",
        pluginAssetUrl: isolatedAssetPath,
      },
      remark:
        "Runtime-hosted isolated iframe entry used by Playwright verification.",
    },
    {
      key: newWindowMenuKey,
      name: newWindowMenuName,
      path: "/extension/runtime-e2e-new-window",
      component: "system/plugin/dynamic-page",
...
test.describe("TC-1 运行时 wasm 插件生命周期", () => {
  ...
  test("TC-1b~g: runtime wasm 上传、安装启用、菜单挂载和卸载资源状态完整链路", async ({
    ...
    await test.step("TC-1e: iframe 菜单在宿主内容区内嵌打开运行时托管页面", async () => {
      ...
      expect(iframeRoute?.component).toBe("#/views/system/plugin/dynamic-page");
      expect(iframeRoute?.path).toBe("/extension/runtime-e2e-iframe");
      expect(iframeRoute?.meta?.pluginId).toBe(pluginID);
      expect(iframeRoute?.meta?.query).toMatchObject({
        pluginAccessMode: "iframe",
        pluginAssetUrl: pluginHostedAssetPath(),
      });
    ...
    await test.step("TC-1f: 新标签页菜单直接打开运行时托管页面", async () => {
      ...
      expect(newWindowRoute?.component).toBe("#/views/system/plugin/dynamic-page");
      expect(newWindowRoute?.meta?.pluginId).toBe(pluginID);
      expect(newWindowRoute?.meta?.query).toMatchObject({
        pluginAccessMode: "new-window",
        pluginAssetUrl: pluginHostedAssetPath(),
      });
      ...
      await expect(page).toHaveURL(/\/extension\/runtime-e2e-new-window(?:\/)?$/);
      await popup.close();
    });

    await test.step("TC-1g: 第二个 iframe 菜单保持同插件同版本资产隔离", async () => {
      const isolatedRoute = findRouteByTitle(routes, isolatedMenuName);
      expect(isolatedRoute, "启用后应生成宿主隔离动态路由").toBeTruthy();
      expect(isolatedRoute?.component).toBe(
        "#/views/system/plugin/dynamic-page",
      );
      expect(isolatedRoute?.meta?.query?.pluginAccessMode).toBe(
        "iframe",
      );
      expect(isolatedRoute?.meta?.query?.pluginAssetUrl).toBe(isolatedAssetPath);
      expect(isolatedRoute?.meta?.pluginId).toBe(pluginID);

      await pluginPage.clickSidebarMenuItem(isolatedMenuName);
      await expect(
        pluginPage.pluginHostedFrame().getByRole("heading", { name: pluginName }),
      ).toBeVisible();
      await expect(
        pluginPage.pluginHostedFrame().getByText("runtime frontend asset", { exact: true }),
      ).toBeVisible();
      expect(
  ...
  test("TC-1h: 独立的 linapro-demo-dynamic 菜单页会展示按钮并打开纯静态独立页面", async ({
    ...
    await expect(page.locator("iframe.hosted-plugin-frame")).toBeVisible();
    ...
    await expect(
      pluginPage.pluginHostedFrame().getByText("动态加载").first(),
    ).toBeVisible();
    await expect(
    ...
    await expect(
      popup.getByRole("heading", {
        name: /动态插件示例已生效|Dynamic Plugin Demo Is Live/,
      }),
    ).toBeVisible();
    await expect(
      popup.getByText(
        /该页面运行于隔离 iframe|This page runs in an isolated iframe/,
      ),
    ).toBeVisible();
    await popup.close();
  });
  ...
  test("TC-1i: 运行时产物被手动删除后列表仍保留条目、菜单隐藏且拒绝同版本重新上传", async ({
    ...
    await pluginPage.expectSidebarMenuHidden(isolatedMenuName);
