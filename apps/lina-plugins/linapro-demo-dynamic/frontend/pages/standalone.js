...
const bridgeProtocol = "linapro.plugin-bridge";
...
function bridgeSessionFromFragment() {
  const parameters = new URLSearchParams(window.location.hash.replace(/^#/, ""));
  const generation = Number(parameters.get("generation"));
  const nonce = parameters.get("nonce") || "";
  const pluginId = parameters.get("pluginId") || "";
  const protocol = parameters.get("protocol") || "";
  const version = Number(parameters.get("version"));
  if (
  ...
  return { generation, nonce, pluginId, protocol, version };
}

function runtimeMessage(messages, key) {
  let value = messages;
  for (const segment of key.split(".")) {
    if (!isPlainObject(value) || !(segment in value)) return "";
    value = value[segment];
  }
  return typeof value === "string" ? value : "";
}

class HostedBridgeClient {
  constructor(session) {
  ...
  envelope(type, extra = {}) {
    return { ...this.session, type, ...extra };
  }

  post(message, transfers = []) {
    window.parent.postMessage(message, "*", transfers);
  }
  ...
  async connect() {
    ...
    const hello = () => this.post(this.envelope("hello"));
    hello();
    const retry = window.setInterval(hello, 250);
    const timeout = window.setTimeout(() => {
      this.readyReject?.(new Error("The LinaPro plugin bridge did not respond"));
  ...
  receive(event) {
    ...
    if (
      ...
      message.protocol !== this.session.protocol ||
      message.version !== this.session.version ||
      message.nonce !== this.session.nonce ||
      message.pluginId !== this.session.pluginId ||
      message.generation !== this.session.generation
    ) {
      return;
    }
    if (message.type === "ready" && isPlainObject(message.context)) {
      this.context = message.context;
      this.readyResolve?.(this.context);
      return;
    }
    ...
    if (message.error) {
      pending.reject(new Error(message.error.message || "Plugin bridge request failed"));
      return;
    }
  ...
  async request(path, options = {}) {
    const requestId = `demo-${Date.now().toString(36)}-${(++this.sequence).toString(36)}`;
    let body = options.body;
    if (typeof body === "string") {
      try {
        body = JSON.parse(body);
      } catch (_error) {
        throw new TypeError("The dynamic demo only sends JSON request bodies");
      }
    }
    const response = new Promise((resolve, reject) => {
      const timeout = window.setTimeout(() => {
        this.pending.delete(requestId);
        this.post(this.envelope("cancel", { requestId }));
        reject(new Error("Plugin bridge request timed out"));
      }, bridgeRequestTimeoutMs);
      this.pending.set(requestId, { reject, resolve, timeout });
    });
    this.post(this.envelope("request", {
      request: {
        body,
  ...
  async requestJSON(path, options = {}) {
    ...
    if (!response.ok) {
      const payload = isPlainObject(response.body) ? response.body : {};
      throw new Error(
        payload.message || payload.error || `Request failed (${response.status || 0})`,
      );
    }
  ...
  async requestBlob(path) {
    const response = await this.request(path, { kind: "blob" });
    if (!response.ok || !(response.body instanceof ArrayBuffer)) {
      throw new Error(`Attachment download failed (${response.status || 0})`);
    }
    return {
  ...
  dispose() {
    ...
    for (const [requestId, pending] of this.pending) {
      window.clearTimeout(pending.timeout);
      this.post(this.envelope("cancel", { requestId }));
      pending.reject(new Error("Plugin bridge session ended"));
    }
    this.pending.clear();
  }
}
...
function buildPageCopy(context) {
  ...
  return {
    ...
    messages: {
      ...
      bridgeRequired: t(
        "plugin.linapro-demo-dynamic.page.message.bridgeRequired",
        "Protected plugin actions require the LinaPro hosted bridge",
      ),
      fileReadFailed: t(
    ...
    pageDescription: t(
      ...
      "This page runs in an isolated iframe and uses the LinaPro hosted bridge for governed plugin API access.",
    ),
    pageTitle: t(
...
function mount(context) {
  ...
  const allowed = (permission) =>
    permissions.has(`${pluginID}:*`) || permissions.has(permission);
  ...
  function updateActionState() {
    addButton.disabled =
      !allowed(`${pluginID}:record:create`) || state.loading || state.submitting;
  ...
  function renderTable() {
    ...
    if (state.records.length === 0) {
      ...
      const emptyTitle = documentRef.createElement("strong");
      emptyTitle.textContent = pageCopy.emptyTitle;
      const emptyText = documentRef.createElement("span");
      emptyText.textContent = pageCopy.emptyText;
    ...
    for (const [label, width] of [
      [pageCopy.table.headers.title, "24%"],
      [pageCopy.table.headers.content, "30%"],
      [pageCopy.table.headers.attachment, "18%"],
      [pageCopy.table.headers.updatedAt, "16%"],
      [pageCopy.table.headers.actions, "12%"],
    ]) {
    ...
    for (const record of state.records) {
      ...
      if (record.hasAttachment) {
        ...
        downloadButton.disabled = !allowed(`${pluginID}:record:view`);
      ...
      editButton.disabled =
        !allowed(`${pluginID}:record:update`) || state.submitting;
      ...
      deleteButton.disabled =
        !allowed(`${pluginID}:record:delete`) || state.submitting;
  ...
  async function requestJSON(path, options = {}) {
    if (!context.bridge) {
      throw new Error(pageCopy.messages.bridgeRequired);
    }
    return await context.bridge.requestJSON(path, options);
  }
  ...
  async function fetchManifestDemo() {
    ...
    try {
      const payload = await requestJSON("manifest-demo");
      if (state.destroyed) {
        return;
      }
  ...
  async function submitRecord() {
    ...
    try {
      ...
      const path = isEditing
        ? `demo-records/${state.editingRecord.id}`
        : "demo-records";
  ...
  async function deleteRecord(record) {
    ...
    clearFeedback();
    try {
      await requestJSON(`demo-records/${record.id}`, {
        method: "DELETE",
      });
  ...
  async function downloadAttachment(record) {
    ...
    try {
      ...
      const { blob } = await context.bridge.requestBlob(
        `demo-records/${record.id}/attachment`,
      );
...
async function loadPublicRuntimeMessages(locale) {
  try {
    const response = await fetch(
      `/api/v1/i18n/runtime/messages?lang=${encodeURIComponent(locale)}`,
      { credentials: "omit", headers: { Accept: "application/json" } },
    );
    if (!response.ok) return {};
    const payload = await response.json();
    const data = isPlainObject(payload?.data) ? payload.data : payload;
    return isPlainObject(data?.messages) ? data.messages : {};
  } catch (_error) {
    return {};
  }
}

function createPageContext(container, bridge, projectedContext, locale) {
  const messages = isPlainObject(projectedContext?.messages)
  ...
  return {
    ...
    permissions: Object.freeze(
      Array.isArray(projectedContext?.permissions)
        ? [...projectedContext.permissions]
        : [],
    ),
    t(key, fallback) {
      return runtimeMessage(messages, key) || fallback;
    },
  };
}

async function startStandalonePage() {
  const container = document.getElementById("linapro-demo-dynamic-app");
  if (!container) return;
  const session = bridgeSessionFromFragment();
  let bridge = null;
  let projectedContext = {};
  let locale = resolveSupportedLocale(
  ...
  if (session) {
    bridge = new HostedBridgeClient(session);
    try {
      projectedContext = await bridge.connect();
      locale = resolveSupportedLocale(projectedContext.locale || locale);
    } catch (error) {
      container.setAttribute("role", "alert");
      container.textContent = error instanceof Error
        ? error.message
        : "The LinaPro plugin bridge is unavailable";
      bridge.dispose();
      return;
    }
  } else {
    projectedContext = {
      locale,
      messages: await loadPublicRuntimeMessages(locale),
      permissions: [],
    };
  }
  ...
  const controller = mount(
    createPageContext(container, bridge, projectedContext, locale),
  );
