...
test.describe('TC004 登录日志清理', () => {
  ...
  test('TC004a: 点击清空按钮弹出确认对话框', async ({ adminPage }) => {
    ...
    // Cancel to close
    const cancelBtn = modal.getByRole('button', { name: /取\s*消|Cancel/i });
  ...
  test('TC004b: 确认清空操作成功执行', async ({ adminPage }) => {
    ...
    // Confirm through the full Semi modal overlay, including its footer.
    const okBtn = modal.getByRole('button', { name: /确\s*(认|定)|OK|Confirm/i });
    await okBtn.click();
