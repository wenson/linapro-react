...
function visibleDeptRows(rows: DeptRow[], expanded: number[], depth = 0): Array<{ depth: number; row: DeptRow }> { return rows.flatMap((row) => [{ depth, row }, ...(expanded.includes(row.id) ? visibleDeptRows(row.children ?? [], expanded, depth + 1) : [])]); }
...
export default function DeptManagement() {
  ...
  const columns: ColumnProps<DeptRow>[] = [
    ...
    { render: (_, row) => renderActions(row), title: host.t("pages.common.actions"), width: 220 },
  ];
  return <section className="org-core-page" data-testid="org-dept-page"><Typography.Title heading={3}>{host.t("plugin.linapro-org-core.dept.tableTitle")}</Typography.Title><Card><Form<DeptListParams> className="org-core-search" layout="horizontal" onReset={() => setParams({})} onSubmit={setParams}><Form.Input field="name" label={host.t("plugin.linapro-org-core.dept.fields.name")} /><Form.Select field="status" label={host.t("pages.common.status")} optionList={statuses.map((item) => ({ label: item.label, value: Number(item.value) }))} /><Space><Button htmlType="submit" theme="solid" type="primary">{host.t("pages.common.search")}</Button><Button htmlType="reset">{host.t("pages.common.reset")}</Button></Space></Form></Card><Card><div className="org-core-toolbar"><Space><Button onClick={() => setExpanded([])}>{host.t("pages.common.collapse")}</Button><Button onClick={() => setExpanded(deptRowKeys(rows))}>{host.t("pages.common.expand")}</Button>{can(host.permissions, "system:dept:add") ? <Button data-testid="org-dept-add" onClick={() => setEditing({})} theme="solid" type="primary">{host.t("pages.common.add")}</Button> : null}</Space></div><div className="responsive-desktop-table" data-testid="org-dept-table"><Table<DeptRow> columns={columns} dataSource={rows} expandedRowKeys={expanded} loading={loading} onExpand={(open, row) => { if (row && "id" in row) toggle(row as DeptRow, open); }} onRow={(row) => ({ onDoubleClick: () => { if (row?.children?.length) toggle(row); } })} pagination={false} rowKey="id" scroll={{ x: 1050 }} /></div><MobileRecordList testId="org-dept-mobile-list">{visibleDeptRows(rows, expanded).map(({ depth, row }) => <MobileRecordCard key={row.id} testId={`org-dept-mobile-card-${row.id}`}><div style={{ paddingInlineStart: depth * 16 }}><MobileRecordTitle>{row.name}</MobileRecordTitle><MobileRecordFields><MobileRecordField label={host.t("plugin.linapro-org-core.dept.fields.code")} value={row.code} /><MobileRecordField label={host.t("pages.common.status")} value={<Tag color={dictColor(statuses, row.status)}>{dictLabel(statuses, row.status)}</Tag>} /><MobileRecordField label={host.t("pages.common.createdAt")} value={formatTimestamp(row.createdAt, host.locale)} /></MobileRecordFields><MobileRecordActions>{renderActions(row)}</MobileRecordActions></div></MobileRecordCard>)}</MobileRecordList></Card><DeptSideSheet api={api} deptId={editing?.deptId} onClose={() => setEditing(undefined)} onSaved={load} open={editing !== undefined} parentId={editing?.parentId} t={host.t} /></section>;
}
