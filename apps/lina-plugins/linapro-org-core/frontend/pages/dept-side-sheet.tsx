...
export function DeptSideSheet({ api, deptId, onClose, onSaved, open, parentId, t }: {
  ...
  return <SideSheet className="org-core-dept-sheet" closable onCancel={onClose} title={t(deptId ? "plugin.linapro-org-core.dept.drawer.editTitle" : "plugin.linapro-org-core.dept.drawer.createTitle")} visible={open} width="min(640px, 100vw)">{loading || !initial ? <Spin aria-label={t("pages.common.loading")} /> : <Form<DeptValues> className="org-core-drawer-form" initValues={initial} key={`${deptId ?? "new"}-${parentId ?? 0}`} labelPosition="top" onSubmit={submit}>
