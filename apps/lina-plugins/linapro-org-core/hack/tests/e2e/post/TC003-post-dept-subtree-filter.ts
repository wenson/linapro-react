...
test.describe('TC003 岗位管理部门树子部门过滤与数量', () => {
  ...
  test('TC003a: 选择父级部门时岗位列表包含子部门岗位', async ({
    ...
    await adminPage.getByTestId('org-post-table').waitFor({ state: 'visible', timeout: 10000 });
    ...
    const treeNode = adminPage
      .getByTestId('org-post-dept-tree')
      .getByRole('treeitem', { name: new RegExp(deptName) });
  ...
  test('TC003b: 岗位部门树节点标签包含岗位数量', async ({ adminPage }) => {
    ...
    const deptTree = adminPage.getByTestId('org-post-dept-tree').getByRole('tree');
    await expect(deptTree).toBeVisible({ timeout: 10000 });

    const treeNodeTitles = deptTree.getByRole('treeitem');
