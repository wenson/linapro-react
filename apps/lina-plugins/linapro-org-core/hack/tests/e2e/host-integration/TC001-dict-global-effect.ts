...
test.describe('TC001 字典修改全局生效', () => {
  ...
  test('TC001a: 修改字典标签后部门管理页面同步更新', async ({ adminPage }) => {
    ...
    await waitForTableReady(adminPage, '[data-testid="org-dept-table"]');

    // 验证表格状态列中显示修改后的标签（Semi Tag）
    const modifiedTag = adminPage.locator('[data-testid="org-dept-table"] .semi-tag', { hasText: modifiedLabel });
  ...
  test('TC001b: 修改字典标签后用户管理页面查询表单同步更新', async ({ adminPage }) => {
    ...
    await adminPage.waitForLoadState('networkidle');
    await waitForTableReady(adminPage, '[data-testid="user-page"] table');

    // 点击用户状态下拉框
    const statusSelect = adminPage.getByLabel(/状态|Status/i).first();
  ...
  test('TC001c: 还原字典标签确保测试环境干净', async ({ adminPage }) => {
    ...
    const originalTag = adminPage.locator('[data-testid="org-dept-table"] .semi-tag', { hasText: originalLabel });
