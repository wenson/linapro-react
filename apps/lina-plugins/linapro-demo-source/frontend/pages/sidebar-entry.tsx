...
export default function SidebarEntry() {
  ...
  const columns: ColumnProps<DemoRecordItem>[] = [
    ...
    { fixed: "right", render: (_, row) => <Space>{can(host.permissions, "linapro-demo-source:example:update") ? <Button data-testid={`linapro-demo-source-record-edit-${row.id}`} onClick={() => setEditing(row.id)} theme="borderless">{host.t("pages.common.edit")}</Button> : null}{can(host.permissions, "linapro-demo-source:example:delete") ? <Popconfirm content={host.t("plugin.linapro-demo-source.page.messages.deleteConfirm")} onConfirm={() => void remove(row)}><Button data-testid={`linapro-demo-source-record-delete-${row.id}`} theme="borderless" type="danger">{host.t("pages.common.delete")}</Button></Popconfirm> : null}</Space>, title: host.t("pages.common.actions"), width: 180 },
  ];
  return <section className="demo-source-page" data-testid="linapro-demo-source-page"><header><Typography.Title heading={3}>{host.t("plugin.linapro-demo-source.page.tableTitle")}</Typography.Title></header><Card><Form<DemoRecordListParams> className="demo-source-search" layout="horizontal" onSubmit={(values) => setParams((current) => ({ ...current, ...values, pageNum: 1 }))}><Form.Input field="keyword" label={host.t("plugin.linapro-demo-source.page.fields.title")} /><Button htmlType="submit" theme="solid" type="primary">{host.t("pages.common.search")}</Button></Form></Card><Card><div className="demo-source-toolbar">{can(host.permissions, "linapro-demo-source:example:create") ? <Button data-testid="linapro-demo-source-record-add" onClick={() => setEditing("new")} theme="solid" type="primary">{host.t("pages.common.add")}</Button> : null}</div><div data-testid="linapro-demo-source-record-table"><Table<DemoRecordItem> columns={columns} dataSource={rows} loading={loading} pagination={{ currentPage: params.pageNum, onChange: (pageNum) => setParams((current) => ({ ...current, pageNum })), pageSize: params.pageSize, total }} rowKey="id" scroll={{ x: 1100 }} /></div></Card><DemoRecordModal api={api} onClose={() => setEditing(undefined)} onSaved={load} open={editing !== undefined} recordId={editing === "new" ? undefined : editing} t={host.t} /></section>;
}
