...
async function expectFormItemSingleColumn(
  ...
  const metrics = await fieldLocator.evaluate((node) => {
    ...
    const formItem =
      (field.closest("[data-field]") as HTMLElement | null) ??
      field.parentElement ??
      field;
...
test.describe("TC006 英文布局回归", () => {
  ...
  test("TC-2a: 侧栏、页签与个人中心表单在英文环境下保持单行可读", async ({
    ...
    const baseNicknameControlWidth = await readWidth(
      adminPage
        .getByPlaceholder(/Please enter a nickname/i)
        .first()
        .locator("xpath=ancestor::*[contains(@class, 'semi-input-wrapper')][1]"),
    );
    ...
    const passwordCurrentInputWidth = await readWidth(
      adminPage
        .getByPlaceholder(/Please enter the current password/i)
        .first()
        .locator("xpath=ancestor::*[contains(@class, 'semi-input-wrapper')][1]"),
    );
    expect(
    ...
    expect(
      Math.abs(baseNicknameControlWidth - passwordCurrentInputWidth),
  ...
  test("TC-2b: 用户、字典与调度日志页在英文环境下保留稳定的搜索与表头布局", async ({
    ...
    await userPage.goto();
    const pageSizeSelector = adminPage
    ...
    const largestPageSizeOption = adminPage
      .getByRole("option", { name: /Items per page: 100$/ })
      .last();
    await expect(
    ...
    await expect(pageSizeSelector).toContainText("Items per page: 100");
    ...
    const deptTree = adminPage.getByTestId("user-dept-tree");
    ...
    await layoutPage.goto("/system/job-log", {
      tableSelector: '[data-testid="job-log-page"]',
    });
    await expectSingleLine(
    ...
    await expectHeaderSingleLine(
      layoutPage.tableHeader(
        /^Status$/,
        adminPage.getByTestId("job-log-page"),
      ),
    ...
    await expectDescriptionLabelSingleLine(
      builtinDescriptions
        .getByText(/^Handler Reference$/)
        .first(),
      "Job built-in handler reference label",
    );
    await expectDescriptionLabelSingleLine(
      builtinDescriptions
        .getByText(/^Handler Parameters$/)
        .first(),
