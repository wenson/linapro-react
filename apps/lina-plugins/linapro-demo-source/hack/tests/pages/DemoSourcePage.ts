...
export class DemoSourcePage extends PluginPage {
  ...
  pluginSourceRecordRow(title: string): Locator {
    return this.page
      .getByTestId("linapro-demo-source-record-table")
  ...
  async createSourceDemoRecord(
    ...
    if (filePath) {
      ...
      await expect(
        this.pluginSourceRecordModal().locator(".semi-upload-file-list"),
      ).toBeVisible();
    }
  ...
  async deleteSourceDemoRecord(title: string) {
    ...
    const confirmPopover = this.page.locator(".semi-popover-wrapper:visible").last();
