...
test.describe("TC-3 AI 调用日志", () => {
  ...
  test("TC-3a~d: 调用日志支持来源列、调用方法/时间筛选、详情脱敏展示和范围清理", async ({
    ...
    try {
      ...
      const mainContent = adminPage.getByTestId("ai-invocation-logs-page");
      ...
      const row = adminPage
        .getByTestId("ai-invocation-table")
        .getByRole("row")
        .filter({ hasText: fixture.sourcePluginId })
      ...
      await expect(
        adminPage
          .getByTestId("ai-invocation-table")
          .getByRole("row")
          .filter({ hasText: otherSourceFixture.sourcePluginId }),
      ).toHaveCount(0);
