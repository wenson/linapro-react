...
export default function ProviderManagement() {
  ...
  const columns: ColumnProps<Provider>[] = [
    ...
    { fixed: "right", render: (_, row) => renderActions(row), title: host.t("pages.common.actions"), width: 320 },
  ];
  return <section className="ai-core-page" data-testid="ai-provider-management-page">
    ...
    <Card>
      <div className="ai-core-toolbar"><Space>{hasPermission(host.permissions, "ai:provider:create") ? <><Button onClick={() => setProviderId("new")} theme="solid" type="primary">{host.t("plugin.linapro-ai-core.provider.actions.addProvider")}</Button><Button onClick={() => setModelProviderId(0)}>{host.t("plugin.linapro-ai-core.model.actions.addModel")}</Button></> : null}</Space></div>
      <ResponsiveListFeedback empty={!rows.length} emptyLabel={host.t("pages.common.emptyList")} error={Boolean(listError)} errorDetail={listError} errorLabel={host.t("pages.common.loadFailed")} loading={loading} loadingLabel={host.t("pages.common.loading")} onRetry={() => void load()} primaryAction={hasPermission(host.permissions, "ai:provider:create") ? <Button onClick={() => setProviderId("new")} theme="solid" type="primary">{host.t("plugin.linapro-ai-core.provider.actions.addProvider")}</Button> : undefined} retryLabel={host.t("fallback.retry")} testId="ai-provider-list-feedback" />
      {!loading && !listError && rows.length ? <>
        <div className="responsive-desktop-table" data-testid="ai-provider-table"><Table<Provider> columns={columns} dataSource={rows} pagination={{ currentPage: params.pageNum, onChange: (pageNum) => setParams((current) => ({ ...current, pageNum })), pageSize: params.pageSize, total }} rowKey="id" scroll={{ x: 1250 }} /></div>
        <MobileRecordList testId="ai-provider-mobile-list">{rows.map((row) => <MobileRecordCard key={row.id} testId={`ai-provider-mobile-card-${row.id}`}><MobileRecordTitle>{row.name}</MobileRecordTitle><MobileRecordFields><MobileRecordField label={host.t("plugin.linapro-ai-core.provider.fields.endpoint")} value={`${row.enabledEndpointCount ?? 0} / ${row.endpointCount ?? 0}`} /><MobileRecordField label={host.t("plugin.linapro-ai-core.provider.fields.modelCount")} value={`${row.enabledModelCount ?? 0} / ${row.modelCount ?? 0}`} /><MobileRecordField label={host.t("pages.common.status")} value={<Tag color={row.enabled === 1 ? "green" : "grey"}>{host.t(row.enabled === 1 ? "plugin.linapro-ai-core.common.enabled" : "plugin.linapro-ai-core.common.disabled")}</Tag>} /></MobileRecordFields><MobileRecordActions>{renderActions(row)}</MobileRecordActions></MobileRecordCard>)}</MobileRecordList>
      </> : null}
    </Card>
