...
export default function ModelManagement() {
  ...
  const [params, setParams] = useState<ModelListParams>({ pageNum: 1, pageSize: 10 }); const [rows, setRows] = useState<Model[]>([]); const [providers, setProviders] = useState<Provider[]>([]); const [total, setTotal] = useState(0); const [loading, setLoading] = useState(false); const [editing, setEditing] = useState<Model | "new">();
  const load = useCallback(async () => { setLoading(true); try { const result = await api.modelList(params); setRows(result.items); setTotal(result.total); } catch (error) { Toast.error(error instanceof Error ? error.message : String(error)); } finally { setLoading(false); } }, [api, params]);
