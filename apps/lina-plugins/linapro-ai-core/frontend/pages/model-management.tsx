...
export default function ModelManagement() {
  ...
  const columns: ColumnProps<Model>[] = [
    ...
    { fixed: "right", render: (_, row) => renderActions(row), title: host.t("pages.common.actions"), width: 180 },
  ];
  return <section className="ai-core-page" data-testid="ai-model-management-page">
    <header className="ai-core-page-header"><Typography.Title heading={3}>{host.t("plugin.linapro-ai-core.model.tableTitle")}</Typography.Title></header>
    <Card><Form<ModelListParams> className="ai-core-search-form" layout="horizontal" onSubmit={(values) => setParams((current) => ({ ...current, ...compactFilters(values), pageNum: 1 }))}><Form.Input field="keyword" label={host.t("plugin.linapro-ai-core.model.fields.modelName")} /><Form.Select field="providerId" label={host.t("plugin.linapro-ai-core.model.fields.provider")} optionList={providers.map((item) => ({ label: item.name, value: item.id }))} /><Form.Select field="enabled" label={host.t("pages.common.status")} optionList={[{ label: host.t("plugin.linapro-ai-core.common.enabled"), value: 1 }, { label: host.t("plugin.linapro-ai-core.common.disabled"), value: 0 }]} /><Button htmlType="submit" theme="solid" type="primary">{host.t("pages.common.search")}</Button></Form></Card>
    <Card>
      <div className="ai-core-toolbar">{hasPermission(host.permissions, "ai:provider:create") ? <Button onClick={() => setEditing("new")} theme="solid" type="primary">{host.t("plugin.linapro-ai-core.model.actions.addModel")}</Button> : <span />}</div>
      <ResponsiveListFeedback empty={!rows.length} emptyLabel={host.t("pages.common.emptyList")} error={Boolean(listError)} errorDetail={listError} errorLabel={host.t("pages.common.loadFailed")} loading={loading} loadingLabel={host.t("pages.common.loading")} onRetry={() => void load()} retryLabel={host.t("fallback.retry")} testId="ai-model-list-feedback" />
      {!loading && !listError && rows.length ? <>
        <div className="responsive-desktop-table" data-testid="ai-model-table"><Table<Model> columns={columns} dataSource={rows} pagination={{ currentPage: params.pageNum, onChange: (pageNum) => setParams((current) => ({ ...current, pageNum })), pageSize: params.pageSize, total }} rowKey="id" scroll={{ x: 1150 }} /></div>
        <MobileRecordList testId="ai-model-mobile-list">{rows.map((row) => <MobileRecordCard key={row.id} testId={`ai-model-mobile-card-${row.id}`}><MobileRecordTitle>{row.modelName}</MobileRecordTitle><MobileRecordFields><MobileRecordField label={host.t("plugin.linapro-ai-core.model.fields.provider")} value={row.providerName || "-"} /><MobileRecordField label={host.t("plugin.linapro-ai-core.model.fields.protocol")} value={protocolLabel(row.protocol)} /><MobileRecordField label={host.t("plugin.linapro-ai-core.model.fields.endpoint")} value={row.endpointBaseUrl || "-"} /><MobileRecordField label={host.t("pages.common.status")} value={<Tag color={row.enabled === 1 ? "green" : "grey"}>{host.t(row.enabled === 1 ? "plugin.linapro-ai-core.common.enabled" : "plugin.linapro-ai-core.common.disabled")}</Tag>} /></MobileRecordFields><MobileRecordActions>{renderActions(row)}</MobileRecordActions></MobileRecordCard>)}</MobileRecordList>
      </> : null}
    </Card>
    <ModelSideSheet api={api} model={editing === "new" ? undefined : editing} onClose={() => setEditing(undefined)} onSaved={load} open={editing !== undefined} t={host.t} />
  </section>;
}
