...
func TestFilesEmbedUpdatedUploadDefaults(t *testing.T) {
	...
	if !strings.Contains(string(configContent), "maxSize: 200") {
		t.Fatalf("expected packed config template to keep 200MB upload default, got %q", string(configContent))
	}
	...
	if !strings.Contains(string(sqlContent), "'sys.upload.maxSize', '200'") {
		t.Fatalf("expected packed config-management sql to keep 200MB upload default, got %q", string(sqlContent))
	}
}
