...
const defaultLabels: RichTextEditorLabels = { image: "Image", link: "Link", linkPrompt: "URL" };
...
export function RichTextEditor({ labels = defaultLabels, onChange, onUploadImage, placeholder, value }: { labels?: RichTextEditorLabels; onChange(value: string): void; onUploadImage(file: File): Promise<string>; placeholder: string; value: string }) {
  ...
  return <div className="rich-text-editor" data-testid="rich-text-editor"><Space className="rich-text-toolbar"><Button aria-label="Bold" onClick={() => editor?.chain().focus().toggleBold().run()} theme={editor?.isActive("bold") ? "solid" : "borderless"}>B</Button><Button aria-label="Italic" onClick={() => editor?.chain().focus().toggleItalic().run()} theme={editor?.isActive("italic") ? "solid" : "borderless"}>I</Button><Button aria-label="Underline" onClick={() => editor?.chain().focus().toggleUnderline().run()} theme={editor?.isActive("underline") ? "solid" : "borderless"}>U</Button><Button onClick={() => { const href = window.prompt(labels.linkPrompt); if (href) editor?.chain().focus().setLink({ href }).run(); }} theme="borderless">{labels.link}</Button><Button onClick={() => fileRef.current?.click()} theme="borderless">{labels.image}</Button></Space><input accept="image/*" className="visually-hidden" onChange={(event) => void image(event.target.files?.[0])} ref={fileRef} type="file" /><EditorContent editor={editor} /></div>;
}
