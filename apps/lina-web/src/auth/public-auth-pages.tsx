...
function PublicAuthShell({ children, title }: { children: React.ReactNode; title: string }) {
  const { t } = useTranslation();
  return <main className="login-page public-auth-page"><section className="login-panel"><Typography.Title heading={2}>{title}</Typography.Title>{children}<Link data-testid="public-auth-back-to-login" to="/auth/login">{t("auth.backToLogin")}</Link></section></main>;
}

export function RegisterPage({ api, enabled, privacyPolicy = "", termsOfService = "" }: { api: PublicAuthApi; enabled: boolean; privacyPolicy?: string; termsOfService?: string }) {
  const { t } = useTranslation();
  ...
  return <PublicAuthShell title={t("auth.register.title")}><Form<{ email: string; password: string; passwordConfirm: string; username: string }> labelPosition="top" onSubmit={async (values) => {
  ...
  </Form>{error ? <Typography.Text role="alert" type="danger">{error}</Typography.Text> : null}<Modal footer={null} onCancel={() => setPolicy(undefined)} title={policy === "privacy" ? t("auth.register.privacyPolicy") : t("auth.register.termsOfService")} visible={Boolean(policy)}><Typography.Paragraph style={{ whiteSpace: "pre-wrap" }}>{policy === "privacy" ? privacyPolicy : termsOfService}</Typography.Paragraph></Modal></PublicAuthShell>;
}

export function ForgetPasswordPage({ api, enabled }: { api: PublicAuthApi; enabled: boolean }) {
  const { t } = useTranslation(); const [error, setError] = useState(""); const [submitted, setSubmitted] = useState(false);
  if (!enabled) return <Navigate replace to="/auth/login" />;
  return <PublicAuthShell title={t("auth.forgotPassword")}><Form<{ email: string }> labelPosition="top" onSubmit={async ({ email }) => {
...
export function ResetPasswordPage({ api, enabled }: { api: PublicAuthApi; enabled: boolean }) {
  ...
  return <PublicAuthShell title={t("auth.reset.title")}><Form<{ password: string; passwordConfirm: string }> labelPosition="top" onSubmit={async (values) => {
    if (values.password !== values.passwordConfirm) { setError(t("auth.register.passwordMismatch")); return; }
